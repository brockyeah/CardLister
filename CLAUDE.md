...
Because the heartbeat is stamped after a *failure* too (it proves liveness), `stale` says nothing about whether alerts are being delivered — a mailer outage drops every alert while the poller reports perfectly fresh. The poller therefore also carries the last cycle's `alerts_pending` / `alerts_abandoned` counts and a `last_cycle_ok` flag onto `_poller_state`, and `/api/health` reports all three under `poller`. `last_cycle_ok` is `run_poll_cycle`'s `fetch_ok`, never an unconditional `True`: **`fetch_callup_transactions` degrades to `[]` on a network error, which is byte-for-byte what a quiet day returns**, so a cycle through a total MLB Stats API outage completes without raising and would otherwise report clean. The poller passes `strict=True` to be told (it catches the resulting `CallupFetchError` itself, so the module's never-crash guarantee is intact for every other caller). `.github/workflows/health.yml` reads them: **`alerts_abandoned > 0` fails the probe** (an alert past the 48h retry window is never delivered), while held alerts and an errored cycle warn, since the next cycle legitimately clears both. Those three fields are deliberately *not* in the workflow's hard shape check — production runs whatever Railway last deployed, so a body without them is expected during a deploy window and reads as `unknown` rather than red; a value that is present but non-numeric does fail.
...
16. **The pricing `source` string `"mock"` is load-bearing across the repo boundary with no shared fixture.** `frontend/src/lib/pricing.js` refuses to persist a suggested price whose `source === 'mock'` — the only thing keeping the $9.99 placeholder out of saved cards. The backend literal lives in `routers/pricing.py`; `test_pricing_chain.py` pins the backend strings and `pricing.test.js` pins the frontend refusal, but unlike the eBay-title and condition tables there is no fixture both suites read — a backend rename that updates its own test leaves both suites green while the frontend guard silently stops refusing mocks.
