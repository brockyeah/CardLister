...
- **Capture:** every real (non-mock, non-error) scan persists its extraction *as shown to the user* (post-overlay — so overlay deltas aren't re-recorded as corrections on every rescan) to a new `scans` table; the scan response carries `scan_id`; the frontend passes it back on save; the backend diffs extraction vs. saved values (normalized) and records non-empty diffs to a `corrections` table. Corrections are intentionally shared across users (shared inventory): either user's fixes improve both users' scans.
